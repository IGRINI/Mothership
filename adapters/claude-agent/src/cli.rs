...
pub(crate) async fn stream_chat(
    ...
    let stderr = stderr_task.await.unwrap_or_default();
    if !status.success() {
        if let Some(error) = result_error.as_deref().and_then(normalize_claude_error) {
            bail!("{error}");
        }
        bail!("Claude process exited with {status}: {}", stderr.trim());
    }
    if let Some(error) = result_error {
        if let Some(error) = normalize_claude_error(&error) {
            bail!("{error}");
        }
...
/// Claude's native content-search tool. Disabled when Mothership `search_text`
/// is bridged so content search flows through Mothership; native `Glob` remains
/// available for file discovery, alongside supervised listing commands through
/// `run_command`.
const NATIVE_CONTENT_SEARCH_TOOLS: &[&str] = &["Grep"];
...
fn disallowed_tools_for_request(request: &ChatRequest) -> Vec<&'static str> {
    ...
    // Route content search through Mothership while preserving native Glob for
    // file discovery now that listing is handled through run_command.
    if request.tools.iter().any(|tool| tool.name == "search_text") {
        tools.extend_from_slice(NATIVE_CONTENT_SEARCH_TOOLS);
    }
    tools
}
...
fn adapter_instructions(core_instructions: String, bridge_enabled: bool) -> String {
    ...
    let bridge_instructions = "When Mothership MCP tools are available, use them for local command execution and all file changes. Prefer `mcp__mothership__run_command` over direct shell tools, prefer the Mothership file tools (`mcp__mothership__read_file`, `mcp__mothership__write_file`, `mcp__mothership__edit_file`, `mcp__mothership__apply_patch`) over native file tools, and use `mcp__mothership__search_text` for supervised content search. For file discovery, use Claude's native Glob or a supervised `mcp__mothership__run_command` listing command. Only fall back to other provider-native tools when no Mothership tool can perform the task.";
...
fn normalize_claude_error(error: &str) -> Option<String> {
    let lower = error.to_ascii_lowercase();
    if lower.contains("api error: 401") || lower.contains("invalid authentication credentials") {
        return Some(
            "Claude Agent SDK rejected the current credentials (401). Use a token generated by `claude setup-token`, or run `claude auth login` again. Raw `accessToken`/`refreshToken` values from `.credentials.json` are not valid `CLAUDE_CODE_OAUTH_TOKEN` values."
                .to_string(),
        );
    }
    if lower.contains("api error: 403") || lower.contains("request not allowed") {
        return Some(
            "Claude Agent SDK recognized the token, but this request is not allowed (403). Use a token generated by `claude setup-token`; pasted `.credentials.json` access/refresh tokens are not enough for headless Agent SDK requests."
                .to_string(),
        );
    }
    None
...
mod tests {
    ...
    #[test]
    fn disallows_native_content_search_when_search_text_present() {
        // Mothership search_text replaces native Grep, while Glob stays enabled
        // for file discovery.
        let request = request_with_tools(&["search_text"]);
        let disallowed = disallowed_tools_for_request(&request);
        assert!(disallowed.contains(&"Grep"));
        assert!(
            !disallowed.contains(&"Glob"),
            "Glob must stay enabled for file discovery"
        );
    }

    #[test]
    fn keeps_native_search_tools_when_search_text_absent() {
        // No typed content-search tool: native Glob/Grep stay enabled.
        let request = request_with_tools(&["run_command", "read_file"]);
        let disallowed = disallowed_tools_for_request(&request);
        assert!(!disallowed.contains(&"Glob"));
        assert!(!disallowed.contains(&"Grep"));
    }

    #[test]
    fn normalizes_claude_auth_errors() {
        let error = normalize_claude_error(
            "Failed to authenticate. API Error: 401 Invalid authentication credentials",
        )
        .expect("normalized error");

        assert!(error.contains("claude setup-token"));
